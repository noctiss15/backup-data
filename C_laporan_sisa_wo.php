...
class C_laporan_sisa_wo extends MY_Controller {
	...
	public function view($type, $id = NULL){
		...
		if ($type == 1) {
			$data = array(
				...
				'title_page' 	=> 'Laporan',
				'title_page2' 	=> 'Laporan Sisa WO',
				);
			$this->open_page('manufaktur/laporan-sisa-wo/V_laporan_sisa_wo', $data);
		} else if ($type == 2) {
			$data = array(
				'aplikasi'		=> $this->app_name,
				'title_page' 	=> 'Laporan',
			...
			$this->open_page('manufaktur/laporan-sisa-wo/V_laporan_laba_kotor', $data);
			// $where['data'][] = array(
			// 	'column' => 'partner_id',
			// 	'param'	 => $id
			// );
			...
			$this->open_page('manufaktur/laporan-sisa-wo/V_laporan_sisa_wo2', $data);
		} 
		// if($priv['read'] == 1)
		// {
	...
	public function loadData($type){
		...
		//WHERE LIKE
		 $where_like['data'][] = array(
		 	'column' => 'order_tanggal, partner_nama',
		...
		if($type == 1){
			$query_total = $this->mod->select('*', 'v_laporan_sisa_wo', null, $where);
			$query_filter = $this->mod->select('*', 'v_laporan_sisa_wo', NULL, $where, NULL, $where_like, $order);
			$query = $this->mod->select('*', 'v_laporan_sisa_wo', NULL, $where, NULL, $where_like, $order, $limit);
			...
			if ($query<>false) {
				foreach ($query->result() as $val) {
					...
					if($val->order_ppn == 10){
						// YA
						$total = $val->orderdet_harga_satuan * $val->orderdet_qty;
						$dpp = $val->orderdet_harga_satuan * 1.1;
						$ppn = $val->orderdet_harga_satuan * 10 / 100;
					} else if($val->order_ppn == 0){
						// TIDAK
						$total = $val->orderdet_harga_satuan * $val->orderdet_qty;
						$dpp = $val->orderdet_harga_satuan / 1.1;
						$ppn = $val->orderdet_harga_satuan - $dpp;
					}
						...
						$response['data'][] = array(
							...
							// 'barang_ukuran' 				=> $val->barang_ukuran,
							'order_type'					=> $val->order_type,
							'orderdet_qty'					=> number_format($val->orderdet_qty, 2, '.', ','),
							'orderdet_harga_satuan'			=> number_format($val->orderdet_harga_satuan, 2, '.', ','),
							'dpp'							=> 'Rp '.number_format($dpp, 2, '.', ','),
							'ppn'							=> 'Rp '.number_format($ppn, 2, '.', ','),
							'total'							=> 'Rp '.number_format($total, 2, '.', ','),
							//'po_customerdet_total'			=> number_format(number_format($val->po_customerdet_harga_satuan, 2, '.', ',') * number_format($val->po_customerdet_qty, 2, '.', ',')), 
							'order_alasan_pembatalan'		=> $val->order_alasan_pembatalan,
	...
	public function cetakPDF($cabang){
		...
		$name = 'Laporan Sisa WO '.date('d-m-Y');
		// if($type == 1) {
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				...
				// 	}
				// 	// END CARI DETAIL
					$url = 'manufaktur/print/P_laporan_sisa_order';
				// } else {
