...
class C_laporan_mutasi_bg extends MY_Controller {
	...
	public function view($type, $id = NULL){
		...
		if ($type == 1) {
			$data = array(
				...
				'title_page' 	=> 'Laporan',
				'title_page2' 	=> 'Laporan Mutasi BG',
				);
			$this->open_page('manufaktur/laporan-mutasi-bg/V_laporan_mutasi_bg', $data);
		} else if ($type == 2) {
			$data = array(
				'aplikasi'		=> $this->app_name,
				'title_page' 	=> 'Laporan',
			...
			$this->open_page('manufaktur/laporan-mutasi-bg/V_laporan_laba_kotor', $data);
			// $where['data'][] = array(
			// 	'column' => 'partner_id',
			// 	'param'	 => $id
			// );
			...
			$this->open_page('manufaktur/laporan-mutasi-bg/V_laporan_mutasi_bg2', $data);
		} 
		// if($priv['read'] == 1)
		// {
	...
	public function loadData($type){
		...
		if($type == 1){
			$query_total = $this->mod->select('*', 'v_laporan_mutasi_bg', null, $where);
			$query_filter = $this->mod->select('*', 'v_laporan_mutasi_bg', NULL, $where, NULL, $where_like, $order);
			$query = $this->mod->select('*', 'v_laporan_mutasi_bg', NULL, $where, NULL, $where_like, $order, $limit);
			...
			if ($query<>false) {
				foreach ($query->result() as $val) {
					...
					$response['data'][] = array(
						'no' => $no,
							'bukti_bgcek_nomor' 					=> number_format($val->bukti_bgcek_nomor, 2, '.', ','),
							'bukti_bgcek_catatan' 					=> $val->bukti_bgcek_catatan,
							'bank_nama' 							=> $val->bank_nama,
							'coa_kode'					    		=> number_format($val->coa_kode, 2, '.', ','),
							'payment_request_piutangdet_jumlah'		=> number_format($val->payment_request_piutangdet_jumlah, 2, '.', ','),
	...
	public function cetakPDF($cabang){
		...
		$name = 'Laporan Mutasi BG '.date('d-m-Y');
		// if($type == 1) {
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				...
				// 	}
				// 	// END CARI DETAIL
					$url = 'manufaktur/print/P_laporan_mutasi_bg';
				// } else {
